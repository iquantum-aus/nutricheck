<div class="selectedFactors index">
	<h2><?php echo __('Selected Factors'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('performed_check_id'); ?></th>
			<th><?php echo $this->Paginator->sort('factor_id'); ?></th>
			<th><?php echo $this->Paginator->sort('created'); ?></th>
			<th><?php echo $this->Paginator->sort('modified'); ?></th>
			<th><?php echo $this->Paginator->sort('status'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php foreach ($selectedFactors as $selectedFactor): ?>
	<tr>
		<td><?php echo h($selectedFactor['SelectedFactor']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($selectedFactor['PerformedCheck']['id'], array('controller' => 'performed_checks', 'action' => 'view', $selectedFactor['PerformedCheck']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($selectedFactor['Factor']['name'], array('controller' => 'factors', 'action' => 'view', $selectedFactor['Factor']['id'])); ?>
		</td>
		<td><?php echo h($selectedFactor['SelectedFactor']['created']); ?>&nbsp;</td>
		<td><?php echo h($selectedFactor['SelectedFactor']['modified']); ?>&nbsp;</td>
		<td><?php echo h($selectedFactor['SelectedFactor']['status']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $selectedFactor['SelectedFactor']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $selectedFactor['SelectedFactor']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $selectedFactor['SelectedFactor']['id']), null, __('Are you sure you want to delete # %s?', $selectedFactor['SelectedFactor']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Selected Factor'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Performed Checks'), array('controller' => 'performed_checks', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Performed Check'), array('controller' => 'performed_checks', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Factors'), array('controller' => 'factors', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Factor'), array('controller' => 'factors', 'action' => 'add')); ?> </li>
	</ul>
</div>
